...
export default function Settings() {
  ...
  return (
    <div className="flex h-full bg-zinc-900/95 text-zinc-100">
        ...
        {PANES.map((p) => (
          <button
            ...
            className={`flex w-full items-center gap-2 rounded-lg px-2.5 py-1.5 text-left text-[13px] font-medium ${
              pane === p ? "bg-blue-500 text-white" : "hover:bg-white/10"
            }`}
          >
      ...
      <main className="min-w-0 flex-1 overflow-y-auto p-6">
              ...
              {(["dark", "light"] as Appearance[]).map((a) => (
                <button
                  ...
                  className={`overflow-hidden rounded-2xl ring-2 transition ${
                    appearance === a ? "ring-blue-500" : "ring-white/10 hover:ring-white/30"
                  }`}
                >
                  <div
                  ...
                  <div className="bg-black/30 py-1.5 text-center text-xs font-medium capitalize">{a}</div>
                </button>
              ))}
            </div>
          </div>
              ...
              {WALLPAPERS.map((w) => (
                <button
                  ...
                  className={`group overflow-hidden rounded-xl ring-2 transition ${
                    wallpaper === w.id ? "ring-blue-500" : "ring-white/10 hover:ring-white/30"
                  }`}
                >
                  ...
                  <div className="flex items-center justify-between bg-black/30 px-2 py-1.5">
                    ...
                    {wallpaper === w.id && <span className="text-xs text-blue-400">✓</span>}
                  </div>
                </button>
              ))}
            </div>
        ...
        {pane === "About" && (
          <div className="flex h-full flex-col items-center justify-center text-center">
            <div className="grid h-20 w-20 place-items-center rounded-full bg-gradient-to-b from-sky-400 to-blue-600 text-3xl font-bold">
              J
            </div>
            <h1 className="mt-4 text-xl font-bold">Jack's MacBook Pro</h1>
            <p className="mt-1 text-sm text-zinc-400">jackOS 26 “Tahoe” · Portfolio Edition</p>
            ...
            <p className="mt-6 max-w-sm text-xs leading-relaxed text-zinc-500">
              ...
              <a className="text-sky-400 hover:underline" href="https://github.com/vivek9patel/vivek9patel.github.io" target="_blank" rel="noopener">
